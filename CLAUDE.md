...
- **macOS runtime gotcha (SIP vs `DYLD_LIBRARY_PATH`)**: rcl dlopens interface typesupport dylibs by leaf name, and SIP strips `DYLD_*` env vars whenever a protected binary (`/bin/sh`, `/usr/bin/env`) execs — including inside pixi's activation capture — so the `DYLD_LIBRARY_PATH` from `install/setup.bash` never reaches node processes and workspace-built interface packages (e.g. `rosetta_interfaces`) fail with `Could not load library lib*__rosidl_typesupport_fastrtps_c.dylib`. Fixed by a darwin-guarded block in `scripts/activate.sh` that symlinks `install/lib/*__rosidl_*.dylib` into the env's `lib/` (which is on `librcl`'s rpath). Re-runs on every activation, so it self-heals after rebuilds and env re-installs.
